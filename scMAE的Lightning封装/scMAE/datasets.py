...
class scRNADataset(Dataset):
    ...
    # 把scRNADataset.__getitem__改成返回Tensor的形式，方便后续训练时直接使用
    def __getitem__(self, index):
        sample, cluster = self.iterator(index)
        return torch.FloatTensor(sample),torch.tensor(cluster, dtype=torch.long)
...
# Lightining封装加载数据集的类
class ScMAEDataModule(pl.LightningDataModule):
    def __init__(self, args):
        super().__init__()
        self.args=args

    def setup(self,stage=None):
        self.train_dataset= scRNADataset(self.args, self.args["dataset"], mode="train")
        self.val_dataset=scRNADataset(self.args, self.args["dataset"], mode="test")
