# main作为Lighting入口，用Trainer代替手写循环
import pytorch_lightning as pl
from pytorch_lightning.callbacks import ModelCheckpoint
from datasets import ScMAEDataModule
from model import ScMAELightning

def main(args):
    dm=ScMAEDataModule(args)
    model=ScMAELightning(args)

    checkpoint_callback=ModelCheckpoint(
        dirpath=args["paths"]["results"],
        filename="scMAE-{epoch:02d}-{var_loss:.4f}",
        save_top_k=1,
        monitor="val_loss",
        mode="min",
    )
